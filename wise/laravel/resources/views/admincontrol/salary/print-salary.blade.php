...
<body>
  ...
  <hr>
  <form class="salary-print-preview" action="" method="get" >
    <table>
      <tr>
        <td style="border: 0;">
  ...
  <div id="customer">
    @if ($results)
      ...
      <h3 style="margin: 15px 0;">
        Salary For: {{ date("F, Y", strtotime("{$year}-{$month}")) }}
      </h3>
      <table style="width: 100%">
        ...
        @foreach ($results as $key => $value)
          <tr>
            ...
            <td>{{ number_format($value->overtime, 2) }}</td>
            <td>{{ number_format($value->advance, 2) }}</td>
            <td>{{ number_format($value->penalty, 2) }}</td>
            <td>
              {{ number_format($value->amount + $value->bonus + $value->overtime - $value->advance - $value->penalty, 2) }}
            </td>
          </tr>
          @php
          $net += $value->amount;
          $payable += ($value->amount + $value->bonus + $value->overtime - $value->advance - $value->penalty);
          @endphp
        @endforeach
        <tr>
          <td colspan="8" align="right" style="font-weight: bold; font-size: 16px;">
            Net Salary: {{ number_format($net, 2) }} <br />
            Payable Salary: {{ number_format($payable, 2) }} <br />
          </td>
        </tr>
      </table>
    @endif
